<?php
/*
Plugin Name: Ajax WordPress Importer
Plugin URI: http://wordpress.org/extend/plugins/ajax-wordpress-importer/
Description: Import posts, pages, comments, custom fields, categories, tags and more from a WordPress export file, using ajax, so runs better.
Author: webord
Author URI: http://webord.net/
Version: 0.6
Text Domain: ajax-import
...
.media-item .progress { position:relative; }
...
(function ($){
	$(document).ready(function () {
		var wp_import = {
			...
			posts: {
				last: 0,
				total: <?php echo count($this->posts); ?>,
				fetch: <?php echo ($this->fetch_attachments?'true':'false'); ?>,
				errors: {},
				import: function () {
					...
					$.ajax({
						...
						data: {
							...
							'import_id': wp_import.id,
							'last': this.last,
							'total': this.total,
							'fetch': this.fetch,
							'user_new': <?php echo "'".json_encode($_POST['user_new'])."'"; ?>,
							'user_map': <?php echo "'".json_encode($_POST['user_map'])."'"; ?>,
							'imported_authors': <?php echo "'".json_encode($_POST['imported_authors'])."'"; ?>
						},
						success: function ( data, status ) {
							console.log(data);
							wp_import.posts.last+=data.done;
							wp_import.posts.import();
							wp_import.progress(wp_import.posts.last, wp_import.$.progress_posts);
						},
			...
			terms: {
				...
				import: function (){
					...
					$.ajax({
						...
						cache: false,
						data: {
							'action': 'process_terms',
							'import_id': wp_import.id,
						},
						success: function ( data, status ) {
							console.log(data);
							var howmany = (
									(data.process.cat !== null?data.process.cat.length:0)+
									(data.process.tag !== null?data.process.tag.length:0)+
									(data.process.term !== null?data.process.term.length:0)
								);
							wp_import.progress(howmany, wp_import.$.progress_terms);
							wp_import.posts.import();
						},
						complete: function ( response, status ) {
							try {
			...
			$: {
				'progress_posts': $('#progress-posts'),
				'progress_terms': $('#progress-terms')
			},
			init: function () {
				this.terms.import();
			},
			progress: function( done, $this ) {
				var	total = parseInt( $this.find( '.state span.t' ).text() ),
					p = Math.floor(done/(total/100));
				if( total == done && p != 100 )
					p = 100;
				$this.find( '.bar' ).animate( { 'width': (200 * done) / total }, 50, function (){
					$this.find( '.state span.d' ).html( done ).end().find( '.percent' ).html( p + '%' );
				} );
		...
		echo
			"<div id='media-items'>" .
				"<div class='media-item' id='progress-terms'>" .
					"<div class='progress'><div class='percent'>0%</div><div class='state'><span class='d'>0</span> of <span class='t'>" . (count($this->terms)+count($this->categories)+count($this->tags)) . "</span></div><div class='bar'></div></div>" .
					"<div class='filename original'>Terms import progress</div>" .
				"</div>" .
				"<div class='media-item' id='progress-posts'>" .
					"<div class='progress'><div class='percent'>0%</div><div class='state'><span class='d'>0</span> of <span class='t'>" . count($this->posts) . "</span></div><div class='bar'></div></div>" .
					"<div class='filename original'>Posts import progress</div>" .
				"</div>" .
			"</div>";
		...
		// $this->process_posts(); // I do the Process Post each Ajax Call it's better to make it work with a lot of images
	}
	...
	function ajax_import_start(){
		$this->fetch_attachments = ( isset($_POST['fetch']) && $_POST['fetch'] && $this->allow_fetch_attachments() );
		$this->id = (int) $_POST['import_id'];
	
		$file = get_attached_file( $this->id );
		if ( ! is_file($file) ) {
			return new WP_Error( 'file-does-not-exists', __( 'The file does not exist, please try again.', 'wordpress-importer' ), array( 'file' => $file, 'id' => $this->id ) );
		}

		$import_data = $this->parse( $file );
		if ( is_wp_error( $import_data ) ) {
			$import_data->add( 'xml-parse-error', __( 'The file could not be parsed, please check the file and try again.', 'wordpress-importer' ), array( 'file' => $file, 'id' => $this->id ) );
			return $import_data;
		}

		if(isset($_POST['user_new']))
			$_POST['user_new'] = json_decode($_POST['user_new']);
		if(isset($_POST['user_map']))
			$_POST['user_map'] = json_decode($_POST['user_map']);
		if(isset($_POST['imported_authors']))
			$_POST['imported_authors'] = json_decode($_POST['imported_authors']);


		$this->last = isset($_POST['last'])? (int) $_POST['last']:0;
	...
	function ajax_import_end() {
		...
		//wp_import_cleanup( $this->id ); // Not right now
...
<?php
	...
	function ajax_process_terms() {
		...
		$res->process = (object) array(
			'cat' => $this->process_categories(),
			'tag' => $this->process_tags(),
			'term' => $this->process_terms(),
		);
	...
	function process_categories() {
		...
		$res = array();
		foreach ( $this->categories as $cn => $cat ) {
			$res[$cn] = new stdClass;
			$res[$cn]->error = false;
			$res[$cn]->exist = false;
			$res[$cn]->success = false;
			...
			if ( ! is_wp_error( $id ) ) {
				...
				if ($res[$cn]->error===false || !is_wp_error($res[$cn]->error))
					$res[$cn]->error = new WP_Error;
				$res[$cn]->error->add(
					'failed-to-import',
					sprintf( 
						__( 'Failed to import category %s', 'wordpress-importer' ),
						esc_html($cat['category_nicename'])
					).( defined('IMPORT_DEBUG') && IMPORT_DEBUG ? ': ' . $id->get_error_message():'' ),
					array( 'tag_number' => $cn, 'tag' => $tag ) 
				);
				continue;
			}
		}
	...
	function process_tags() {
		...
		$res = array();
		foreach ( $this->tags as $tn => $tag ) {
			$res[$tn] = new stdClass;
			$res[$tn]->error = false;
			$res[$tn]->exist = false;
			$res[$tn]->success = false;
			...
			if ( ! is_wp_error( $id ) ) {
				...
				if ($res[$tn]->error===false || !is_wp_error($res[$tn]->error))
					$res[$tn]->error = new WP_Error;
				$res[$tn]->error->add(
					'failed-to-import',
					sprintf( 
						__( 'Failed to import post tag %s', 'wordpress-importer' ),
						esc_html($tag['tag_name'])
					).( defined('IMPORT_DEBUG') && IMPORT_DEBUG ? ': ' . $id->get_error_message():'' ),
					array( 'tag_number' => $tn, 'tag' => $tag ) 
				);
				continue;
			}
		}
			...
			if ( ! is_wp_error( $id ) ) {
				...
				$res[$tn]->error->add(
					...
					sprintf( 
						...
						esc_html($term['term_taxonomy']), esc_html($term['term_name'])
					).( defined('IMPORT_DEBUG') && IMPORT_DEBUG ? ': ' . $id->get_error_message():'' ),
					array( 'term_number' => $tn, 'term' => $term ) 
				);
				continue;
			}
		}
	...
	function ajax_process_posts() {
		...
		/*$res->posts = */$this->posts = $posts;
		$res->done = $i-$this->last;

		$res->process = $this->process_posts();
	...
	function process_posts() {
		$res = array();
		foreach ( $this->posts as $pn => $post ) {
			$res[$pn] = new stdClass;
			$res[$pn]->error = false;
			$res[$pn]->success = false;
			if ( ! post_type_exists( $post['post_type'] ) ) {
				if ($res[$pn]->error===false || !is_wp_error($res[$pn]->error))
					$res[$pn]->error = new WP_Error;
				$res[$pn]->error->add(
					'failed-to-import',
					sprintf( 
					...
					array( 'post_number' => $pn, 'post' => $post ) 
				);
				continue;
			}
				...
				if ( is_wp_error( $post_id ) ) {
					...
					$res[$pn]->error->add(
						...
						sprintf(
							__( 'Failed to import %s &#8220;%s&#8221;', 'wordpress-importer' ),
							$post_type_object->labels->singular_name, esc_html($post['post_title'])
						).( defined('IMPORT_DEBUG') && IMPORT_DEBUG ? ': ' . $post_id->get_error_message() : '' ),
						array( 'post_number' => $pn, 'post' => $post ) 
					);
					continue;
				}
...
function wordpress_importer_init() {
	...
	add_action( 'wp_ajax_process_terms', array( $GLOBALS['wp_import'], 'ajax_process_terms' ) );
